...
for (k in 1:K)
{
    ...
    nn.model <- neuralnet(f, data = tr, hidden = c(128, 64), startweights = weights, linear.output = FALSE, lifesign = 'full')
    weights <- nn.model$weights
    toc()
    nn.predict <- compute(nn.model, vl[, pixels.select])

    accuracy[k] <- sum(max.col(nn.predict$net.result) - 1 == digits.train$label[-index])/4200
    print(paste("Mean Accuracy:", mean(accuracy)))
    print(paste("Accuracy Change:", accuracy[k] - accuracy[k-1]))
}
toc()
